...
const getCandles = async (symbol, resolution = "D", from, to) => {
  ...
  if (data["Note"])          throw new Error("Alpha Vantage rate limit hit — try again later");
  if (data["Information"])   throw new Error("Alpha Vantage rate limit hit — try again later");
  ...
  if (!tsKey) throw new Error(`No candle data for ${symbol}`);
  ...
  cache.set(key, result, 3600); // cache 1 hour
  return result;
};
...
const getNews = async (symbol, from, to) => {
  ...
  const result = feed.slice(0, 10).map((item) => ({
    headline:  item.title,
  ...
  cache.set(key, result, 1800); // cache 30 minutes
  return result;
};
